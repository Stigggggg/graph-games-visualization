...
# AI for EF game
# chooses optimal move based on Minimax algorithm
# when there is no optimal move, topological heuristic is used
def get_move(game):
    g1 = game['g1']
    g2 = game['g2']
    ...
    if turn == 'spoiler':
        ...
        for spoiler_graph_string in ['g1', 'g2']:
            ...
            # searching all legal vertices to play
            for v in spoiler_graph.nodes():
                if v in spoiler_moves:
                    continue

                # copy list of moves to simulation
                ...
                # simulation of every possible Duplicator move
                for duplicator_v in duplicator_graph.nodes():
                    if duplicator_v in duplicator_moves:
                        continue

                    duplicator_test1 = list(test1)
                    duplicator_test2 = list(test2)

                    if duplicator_graph_string == 'g1':
                        duplicator_test1.append(duplicator_v)
                    else:
                        duplicator_test2.append(duplicator_v)

                    # if Duplicator survives the round, we check if he can survive all rounds
                    survives, _ = check_iso(g1, g2, duplicator_test1, duplicator_test2)
                    if survives:
                        if duplicator_can_win(g1, g2, duplicator_test1, duplicator_test2, round, max_rounds):
                            can_duplicator_win = True
                            break

                # if Duplicator cant win, v is best move for spoiler
        ...
        # if Spoiler cant win, he will make any legal move
    ...
    # similar duplicator logic
        ...
        for v in game[ai_graph].nodes():
            ...
            # if Duplicator survives after the move, we remember the move to choose best one
            survives, _ = check_iso(g1, g2, test1, test2)

            if survives:
                valid_moves.append(v)
                # if this move leads to a win, it is the best choice
                if duplicator_can_win(g1, g2, test1, test2, round, max_rounds):
                    best_move = v
                    break

        # if there is no game winning move, a heuristic is used
        # its goal is to prolong the game as long as possible
        if best_move is None:
            if valid_moves:
                if spoiler_graph == 'g1':
                ...
                # heuristic will choose a node with degree closest to the last move
            ...
            # if this will not work, Duplicator loses in this round, so he will pick the same color as the Spoiler if possible
            else:
...
def get_pebble_move(game):
    ...
    if turn == 'spoiler':
        ...
        for spoiler_graph_string in ['g1', 'g2']:
            ...
            for pebble_id in pebbles:
                ...
                # simulation of every possible Duplicator move
                for duplicator_v in duplicator_graph.nodes():
                    duplicator_test_p1 = dict(test_p1)
                    duplicator_test_p2 = dict(test_p2)
                    if duplicator_graph_string == 'g1':
                        duplicator_test_p1[pebble_id] = duplicator_v
                    else:
                        duplicator_test_p2[pebble_id] = duplicator_v

                    # we check if duplicator can survive this round
                    survives, _ = check_iso_pebbles(g1, g2, duplicator_test_p1, duplicator_test_p2)
                    if survives:
                        can_duplicator_win = True
                        break
        ...
        if best_move is None:
            ...
            best_move = random.choice(list(g1.nodes()))

        game['spoiler_choice_graph'] = best_graph
        game['current_pebble'] = best_pebble

        if best_graph == 'g1':
            game['pebbles_g1'][best_pebble] = best_move
        else:
            game['pebbles_g2'][best_pebble] = best_move
    ...
    # similar duplicator logic
        ...
        if valid_moves:
            if spoiler_graph == 'g1':
                last_move = game['pebbles_g1'][active_pebble]
            else:
                last_move = game['pebbles_g2'][active_pebble]
