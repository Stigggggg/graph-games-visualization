...
// BaseMenu component, EF and Pebbles menu will reuse it
interface BaseMenuProps {
    title: string; // game title
    children: ReactNode; // game specific UI elements
    onStart: (baseState: BaseMenuState) => Promise<void>; // async callback function triggered when game is starting
    // it receives data from baseState and handles the API and routing
}
...
export function BaseMenu({ title, children, onStart }: BaseMenuProps) {
    ...
    // downloading json file with example graph
        ...
        <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-4 box-border bg-gray-50">
           <Subtitle className="text-3xl">{title}</Subtitle>
            ...
            <Card className="gap-5">
                ...
                {/* random graph generating options */}
                {source === "random" && (
                ...
                {/* back to menu and start game navigating buttons */}
